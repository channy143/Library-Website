...
async function loadUserData() {
    ...
    // Load borrowed books
    const borrowedResult = await apiGet(`borrow?action=list&userId=${userId}`);
    userBorrowed = borrowedResult.success ? borrowedResult.data : [];

    // Load reservations
    const reservationsResult = await apiGet(`reserve?action=user&userId=${userId}`);
    userReservations = reservationsResult.success ? reservationsResult.data : [];

    // Load wishlist
    const wishlistResult = await apiGet(`wishlist?action=list&userId=${userId}`);
...
async function borrowBook(bookId) {
    ...
    const result = await apiPost('borrow?action=borrow', {
        bookId: bookId,
...
async function returnBook(bookId) {
    ...
    const result = await apiPost('borrow?action=return', {
        bookId: bookId,
        userId: currentUser.id
    });
...
async function renewBook(bookId) {
    ...
    const result = await apiPost('borrow?action=renew', {
        bookId: bookId,
        userId: currentUser.id
    });
...
async function reserveBook(bookId) {
    ...
    const result = await apiPost('reserve?action=reserve', {
        bookId: bookId,
        userId: currentUser.id,
...
async function cancelReservation(reservationId) {
    ...
    const result = await apiPost('reserve?action=cancel', {
...
async function toggleWishlist(bookId, event) {
    ...
    const result = await apiPost(`wishlist?action=${action}`, {
        bookId: bookId,
        userId: currentUser.id
    });
...
async function handleBookSubmit(event) {
    ...
    const result = await apiPost('books?action=add', {
