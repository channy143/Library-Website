document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadBookRatingSummary(bookId) {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/ratings.php?action=book&bookId=${bookId}${userParam}`);
            const result = await response.json();
    ...
    async function submitUserRating(value) {
        ...
        try {
            const response = await fetch(`${API_BASE}/ratings.php?action=rate`, {
                method: 'POST',
    ...
    async function borrowBook(adminBook) {
        ...
        try {
            const response = await fetch(`${API_BASE}/borrow.php?action=borrow`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    bookId: adminBook.id,
    ...
    async function pickupReservationById(id) {
        ...
        try {
            const response = await fetch(`${API_BASE}/borrow.php?action=pickup`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    reservationId: id,
    ...
    async function returnBook(bookId) {
        ...
        try {
            const response = await fetch(`${API_BASE}/borrow.php?action=return`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    bookId: bookId,
    ...
    async function renewBook(bookId) {
        ...
        try {
            const response = await fetch(`${API_BASE}/borrow.php?action=renew`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    bookId: bookId,
    ...
    async function loadUserBorrowedBooks() {
        ...
        try {
            const response = await fetch(`${API_BASE}/borrow.php?action=list&userId=${currentUser.id}`);
            const result = await response.json();
    ...
    async function loadUserReservations() {
        ...
        try {
            ...
            const [activeResp, expiredResp] = await Promise.all([
                fetch(`${API_BASE}/reserve.php?action=user&userId=${currentUser.id}`),
                fetch(`${API_BASE}/reserve.php?action=expired&userId=${currentUser.id}`)
            ]);
    ...
    async function loadUserWishlist() {
        ...
        try {
            const response = await fetch(`${API_BASE}/favorites.php?action=list&userId=${currentUser.id}`);
            const result = await response.json();
    ...
    async function loadUserFullStats() {
        ...
        try {
            const response = await fetch(`${API_BASE}/history.php?action=fullStats&userId=${currentUser.id}`);
            const result = await response.json();
            if (result.success && result.data) {
    ...
    // Load recently read books from recent.php
    async function loadRecentlyRead() {
        ...
        try {
            const response = await fetch(`${API_BASE}/recent.php?action=list&userId=${currentUser.id}`);
            const result = await response.json();
            if (result.success && Array.isArray(result.data)) {
    ...
    // Track a book as recently read via recent.php
    async function trackRecentlyRead(bookId) {
        if (!currentUser || !currentUser.id || !bookId) return;

        try {
            await fetch(`${API_BASE}/recent.php?action=track`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userId: currentUser.id,
    ...
    async function toggleWishlist(bookId) {
        ...
        try {
            const response = await fetch(`${API_BASE}/favorites.php?action=${action}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    bookId: bookId,
    ...
    async function loadUserNotificationsAndShowBanner() {
        ...
        try {
            const response = await fetch(`${API_BASE}/notifications.php?action=listForUser&userId=${currentUser.id}`);
            const result = await response.json();
            ...
            try {
                await Promise.all(relevantNudges.map(n => {
                    if (!n.id) return null;
                    return fetch(`${API_BASE}/notifications.php?action=markRead`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
    ...
    // Load extended profile data from profile.php
    ...
    // Save profile changes: update core user fields via users.php
    async function saveUserProfile(buttonEl) {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Save extended profile info (phone/bio) via profile.php
                try {
    ...
    if (modalBorrowerBtn && reservationDetails && reservationList) {
        modalBorrowerBtn.addEventListener('click', async () => {
            ...
            try {
                const resp = await fetch(`${API_BASE}/borrow.php?action=bookBorrowers&bookId=${currentAdminBook.id}`);
                ...
                for (const borrower of borrowers) {
                    ...
                    if (isOverdue && borrower.userId) {
                        try {
                            const profileResp = await fetch(`${API_BASE}/profile.php?userId=${borrower.userId}`);
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (contactBtn) {
            ...
            try {
                const response = await fetch(`${API_BASE}/notifications.php?action=borrowerNudge`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
