...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function loadReservations() {
        try {
            ...
            for (const user of users) {
                const response = await fetch(`${API_BASE}/reserve?userId=${user.id}`);
    ...
    async function loadBookBorrowers(bookId) {
        try {
            const response = await fetch(`${API_BASE}/borrow?action=bookBorrowers&bookId=${bookId}`);
            const result = await response.json();
    ...
    async function loadAdminStats() {
        try {
            const [usersRes, historyRes] = await Promise.all([
                fetch(`${API_BASE}/users?action=stats`),
                fetch(`${API_BASE}/history?action=adminStats`)
            ]);
    ...
    async function saveBook(bookData, bookId = null) {
        try {
            ...
            if (bookId) {
                payload.id = bookId;
                response = await fetch(`${API_BASE}/books?id=${bookId}`, {
                    method: 'PUT',
            ...
            if (userId) {
                ...
                if (registerResult.success && userData.status && userData.status !== 'active') {
                    try {
                        ...
                        if (newId) {
                            await fetch(`${API_BASE}/users?id=${newId}`, {
                                method: 'PUT',
                                headers: { 'Content-Type': 'application/json' },
